 ...
 * github上的鲁班图片压缩修改版本不需要rxjava和rxandroid
 * Created by jiahongfei on 16/9/8.
 */
public class LubanPhoto {
    ...
    private static volatile LubanPhoto INSTANCE;
    ...
    private LubanPhoto(File cacheDir) {
        mCacheDir = cacheDir;
    }
    ...
    private static File getPhotoCacheDir(Context context) {
        return getPhotoCacheDir(context, LubanPhoto.DEFAULT_DISK_CACHE_DIR);
    }
    ...
    public static LubanPhoto get(Context context) {
        if (INSTANCE == null) INSTANCE = new LubanPhoto(LubanPhoto.getPhotoCacheDir(context));
        return INSTANCE;
    }
...
//        checkNotNull(mFile, "the image file cannot be null, please call .load() before this method!");
        ...
        mExecutorService.execute(new Runnable() {
            @Override
            public void run() {
                File compPhotoFile = null;
                try {
                    if (gear == LubanPhoto.FIRST_GEAR) {
                        compPhotoFile = firstCompress(mFile);
                    } else if (gear == LubanPhoto.THIRD_GEAR) {
                        compPhotoFile = thirdCompress(mFile);
                    }
                    if (null == compPhotoFile ||
                            !compPhotoFile.exists()) {
                        throw new NullPointerException("compress result file null");
                    } else {
                        mHandler.sendMessage(mHandler.obtainMessage(0, compPhotoFile));
                    }
                } catch (Throwable e) {
                    e.printStackTrace();
...
//                mHandler.sendMessage(mHandler.obtainMessage(0, compPhotoFile));
                if (null != compressListener) {
                    compressListener.onSuccess(compPhotoFile);
                }
            }
        } catch (Throwable e) {
            e.printStackTrace();
//            mHandler.sendMessage(mHandler.obtainMessage(1, e));
    ...
    public LubanPhoto setCacheDir(File cacheDir) {
        mCacheDir = cacheDir;
        return this;
    }

    public LubanPhoto load(File file) {
        mFile = file;
        return this;
    }

    public LubanPhoto setCompressListener(OnCompressListener listener) {
        compressListener = listener;
        return this;
    }

    public LubanPhoto putGear(int gear) {
        this.gear = gear;
        return this;
    }

    public LubanPhoto setFilename(String filename) {
